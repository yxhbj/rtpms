  ...
  return new Promise((resolve, reject) => {
    Promise.all([getSettings(), getServers()])
      .then(system => {
        ...
        var cmd = `echo "find ${searchVolume} -type f -name '*.tar' -ls" >/tmp/findtar.sh;bash /tmp/findtar.sh;rm /tmp/findtar.sh`;
...
let getLockStatus = async function(settings) {
  var servers = await getServers(),
    ...
    cmd =
      "echo 'grep PlanLockStatus /usr/local/adacnew/Patients/Institution_*/Mount_*/Patient_*/Plan_*/plan.PlanInfo' > /tmp/cmd.sh;bash /tmp/cmd.sh;rm /tmp/cmd.sh";
  return new Promise(function(resolve, reject) {
    execCommands(config, [cmd])
      .then(res => {
        var arr = res[0].split("\r\n");
...
async function updateBackupPendingList() {
  schedule.scheduleJob("0 * * * * *", () => {
    getSettings().then(settings=>{
      var minute = settings.backup.startTime.split(":")[1];
      var hour = settings.backup.startTime.split(":")[0];
      var time = new Date()
      if(Number(hour)===time.getHours()&&Number(minute)===time.getMinutes()){
        exportPendingList();
      }
    }).catch(e => console.log(e));
  });
...
async function transferPendingList(data) {
  ...
  var cmdDel = `rm /tmp/.rtms/backuplist`;
...
function updateImageSets() {
  Promise.all([getSettings(), getServers()])
    .then(system => {
      ...
      // console.log(config)
      var dicomFolder=settings.institution.dicomFolder
      var listDicomFilesCmd=`echo "ls -l ${dicomFolder}/*.dcm" >/tmp/finddcm.sh;bash /tmp/finddcm.sh;rm /tmp/finddcm.sh`
      execCommands(config,[listDicomFilesCmd]).then(result=>{
        // console.log(result[0])
        var dcmFiles=result[0].split("\r\n").map(m=>m.split(/\s+/)[8])
        var dcmFileNames=dcmFiles.map(m=>m.replace(/\/.*\//,"").split("."))
        var imageSets=groupImages(dcmFileNames)
        imageSets.forEach((img,i)=>{
          parseDicomInfo(img,config,dicomFolder).then(imgInfo=>{
            getRecords("images","mrn",`'${imgInfo.mrn}'`).then(oimg=>{
              if(oimg.length>0) return
              insertRecord("images",imgInfo).then().catch(e=>console.log(e))
            }).catch(e=>console.log(e))
          }).catch(e=>console.log(e))
        })
      }).catch(e=>console.log(e))
...
function parseDicomInfo(images,config,dicomFolder){
  var dicomHeadCmd=`/usr/local/adacnew/PinnacleStatic/bin/common/DICOMList ${dicomFolder}/${images[0].join(".")}`
  return new Promise((resolve,reject)=>{
    execCommands(config,[dicomHeadCmd]).then(res=>{
      var retObj={},imgInfo={}
      JSON.stringify(res[0]).match(/[A-F\d]{4}\,\d{4}.*?\|\\r\\n?(\s*\|.*?\|)?/g).forEach(m=>{
          retObj[m.match(/\s+.*?\s{2,}/g)[0].trim()]=m.match(/\|.*?\|/g).join("").replace(/\|/g,"").trim()
      })
      //console.log(retObj)
      imgInfo.mrn=retObj['Patient ID']
      imgInfo.patientname=retObj['Patient\'s Name']
      imgInfo.modality=retObj['Modality']
      imgInfo.examid=retObj['Accession Number']            
      imgInfo.studyid=retObj['Study ID']
      imgInfo.scantime=convertTimeStamp(retObj['Study Time'])
      imgInfo.bodypart=retObj['Body Part Examined']
      imgInfo.seriesdescription=retObj['Study Description']
      imgInfo.imagenumber=images.length
      resolve(imgInfo)
    }).catch(e=>reject(e))
  })
}
...
function groupImages(list){
  var finalArr=[],tmpArr=list[0]
  list.forEach((m,i,arr)=>{
      var diffItems=0
      m.forEach((n,j)=>{
          if(m[j]!=tmpArr[0][j]) diffItems++
      })
      if(diffItems==1){
          tmpArr.push(m)
      }else if(diffItems>1&&diffItems<m.length){
          finalArr.push(JSON.parse(JSON.stringify(tmpArr)))
          tmpArr=[]
          tmpArr.push(m)
      }else{
          tmpArr=[]
...
function deleteRecord(table,filterField,filterVal,pgConfig) {
  var config=pgConfig?pgConfig:lpgConfig;whereStr=''
  if(filterVal&&filterField){
    whereStr=` WHERE ${filterField} = ${filterVal}`
  }
  var pool = new pg.Pool(config);
  var _query = `DELETE FROM ${table} ${whereStr} `;
  return new Promise((resolve, reject) => {
    pool.connect(function(err, client, done) {
      if (err) {
        reject(err);
