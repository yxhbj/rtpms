...
function patientInit(institutionField) {
  var instFildterOption=institutionField.institutions.map(m=>({value:m.id,text:m.name}))
      ...
      columnData: [
        {
          ...
          filter: {
            ...
            selected: institutionField.defaultInstitution,
            inMultiple:true
          }
        },
        {
            ...
            actionButton.addEventListener("click", function(e) {
              if (
                ...
                if (r==true){
                  ...
                  const xhr = new XMLHttpRequest();
                  xhr.open("POST", "http://localhost:3000/deletePatient");
...
const getImageData = function(paramse) {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", uri+"image");
    xhr.setRequestHeader("Content-Type", "multipart/form-data");
    xhr.onreadystatechange = function() {
      if (xhr.readyState !== 4) {
        return;
...
function init() {
  ...
  xhr.onreadystatechange = function() {
    ...
    if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 304) {
      var result = JSON.parse(xhr.response);
      //console.log(result)
      patientInit(result.institution);
    }
  };
  xhr.send("");
}
